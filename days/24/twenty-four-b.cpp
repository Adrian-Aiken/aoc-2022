...
int main()
{
    ...
    // Perform front search
    int maxLen = 0, fullTime = 0;
    ...
    while (!searches.empty())
    {
        ...
        {
            cout << "Takes " << curPath.size()-1 << " minutes" << endl << endl;
            fullTime += curPath.size() - 1;
            break;
        }
    ...
    // Perform back search
    maxLen = 0;
    visited = set<pair<int, COORD>>();
    searches = queue<vector<COORD>>();
    searches.push(vector<COORD>({ COORD(valley.size()-1, valley[0].size()-2) }));
    while (!searches.empty())
    {
        vector<COORD> curPath = searches.front(); searches.pop();
        COORD curSpot = curPath.back();
        vector<string> curMap = cycles[(fullTime + curPath.size()) % cycles.size()];
        ...
        if (curSpot.first == 0 && curSpot.second == 1)
        {
            cout << "Takes " << curPath.size()-1 << " minutes" << endl << endl;
            fullTime += curPath.size() - 1;
            break;
        ...
        // Down - extra condition to prevent rentering to the entrance
        if (curSpot.first < valley.size()-1 && curMap[curSpot.first+1][curSpot.second] == '.')
        {
            vector<COORD> newPath = curPath;
            newPath.push_back(COORD(curSpot.first+1, curSpot.second));
            searches.push(newPath);
    ...
    cout << endl << "Total round trip: " << fullTime << " minutes" << endl;
}